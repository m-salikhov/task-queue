...
t.test('Executor.executeTask()', async (t) => {
  ...
  t.equal(
    executor.getExecuteData().running[0],
    init0Task,
    '`executor.executeTask(init0Task)` places task into `executor.executeData.running`'
  );

  await init0Promise;
  t.same(executor.getExecuteData().running, {}, 'after execution of all tasks `executor.executeData.running` is empty');
  t.same(
    executor.getExecuteData().completed,
    { [init0Task.targetId]: [init0Task] },
...
t.test('Executor.performanceReport for 4 simultaneous tasks', async (t) => {
  ...
  t.ok(
    executor.getPerformanceReport().min >= 3 && executor.getPerformanceReport().min <= 4,
    '`executor.performanceReport.min` is between `3` and `4` (' + executor.getPerformanceReport().min + ')'
  );
  t.equal(
    executor.getPerformanceReport().max,
    4,
    '`executor.performanceReport.max` `4` (' + executor.getPerformanceReport().max + ')'
  );
  t.ok(
    executor.getPerformanceReport().avg >= 3 && executor.getPerformanceReport().avg <= 4,
...
t.test('Executor.performanceReport for 3 simultaneous tasks + 2 simulataneous tasks after', async (t) => {
  ...
  await Promise.all([executor.executeTask(init3Task), executor.executeTask(init4Task)]);

  executor.stop();

  t.equal(
    executor.getPerformanceReport().min,
    2,
  ...
  t.ok(
    executor.getPerformanceReport().avg >= 2 && executor.getPerformanceReport().avg <= 3,
